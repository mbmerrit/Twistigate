...
di_p = [di_p; 100*(theta_nom-thetas)/theta_nom];
plot(100*(di_nom-di_vary)/di_nom,100*(theta_nom-thetas)/theta_nom,'LineWidth',2); hold on;
...
delta_p = [delta_p; 100*(theta_nom-thetas)/theta_nom];
plot(100*(delta_nom-delta_vary)/delta_nom,100*(theta_nom-thetas)/theta_nom,'LineWidth',2); hold on;
...
Nt_p = [Nt_p; 100*(theta_nom-thetas)/theta_nom];
plot(100*(Nt_nom-Nt_vary)/Nt_nom,100*(theta_nom-thetas)/theta_nom,'LineWidth',2); hold on;
...
Lf_p = [Lf_p; 100*(theta_nom-thetas)/theta_nom];
plot(100*(Lf_nom-Lf_vary)/Lf_nom,100*(theta_nom-thetas)/theta_nom,'LineWidth',2); hold on;
...
nu_p = [nu_p; 100*(theta_nom-thetas)/theta_nom];
plot(100*(nu_nom-nu_vary)/nu_nom,100*(theta_nom-thetas)/theta_nom,'LineWidth',2); hold on;
l{l_int} = '\nu '; l_int = l_int + 1;
%% Vary dw
spring = Convert_Build_Params(spring_nom);
dw_nom = spring.d_w;
...
dw_p = [dw_p; 100*(theta_nom-thetas)/theta_nom];
plot(100*(dw_nom-dw_vary)/dw_nom,100*(theta_nom-thetas)/theta_nom,'LineWidth',2); hold on;
l{l_int} = 'd_w '; l_int = l_int + 1;
end
%% Plot all parameters area plots
figure;
a = area(linspace(-25,25,101),flipud([(min(abs(di_p)).*sign(di_p(1,:)))' (range(di_p).*sign(di_p(1,:)))'])); hold on;
a(1).FaceColor = 'none'; a(1).LineStyle = 'none'; a(2).FaceAlpha = .5;

b = area(linspace(-25,25,101),flipud([(min(abs(delta_p)).*sign(delta_p(1,:)))' (range(delta_p).*sign(delta_p(1,:)))'])); hold on;
b(1).FaceColor = 'none'; b(1).LineStyle = 'none'; b(2).FaceAlpha = .5;

c = area(linspace(-25,25,101),flipud([(min(abs(Nt_p)).*sign(Nt_p(1,:)))' (range(Nt_p).*sign(Nt_p(1,:)))'])); hold on;
c(1).FaceColor = 'none'; c(1).LineStyle = 'none'; c(2).FaceAlpha = .5;

d = area(linspace(-25,25,101),flipud([(min(abs(Lf_p)).*sign(Lf_p(1,:)))' (range(Lf_p).*sign(Lf_p(1,:)))'])); hold on;
d(1).FaceColor = 'none'; d(1).LineStyle = 'none'; d(2).FaceAlpha = .5;

e = area(linspace(-25,25,101),flipud([(min(abs(nu_p)).*sign(nu_p(1,:)))' (range(nu_p).*sign(nu_p(1,:)))'])); hold on;
e(1).FaceColor = 'none'; e(1).LineStyle = 'none'; e(2).FaceAlpha = .5;

f = area(linspace(-25,25,101),flipud([(min(abs(dw_p)).*sign(dw_p(1,:)))' (range(dw_p).*sign(dw_p(1,:)))'])); hold on;
