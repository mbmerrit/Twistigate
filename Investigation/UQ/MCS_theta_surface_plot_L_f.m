...
subplot(142);hold on;
gscatter(X_3*1e3,X_2*1e3,group_idx,[],[],[],'off');
xlabel('$L_f~$(mm)','fontsize',20,'interpreter','latex');
...
subplot(143);hold on;
gscatter(X_3*1e3,X_4,group_idx,[],[],[],'off');
xlabel('$L_f~$(mm)','fontsize',20,'interpreter','latex');
...
subplot(144);hold on;
f1=gscatter(X_3*1e3,X_5,group_idx,[],[],[],'off');
% for k = 1:length(f1)
%     f1(k).MarkerSize = 9;
%     if k == 1
...
savefig(['figures/surface_L_f_vs_parameters.fig'])
