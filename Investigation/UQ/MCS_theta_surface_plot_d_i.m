...
subplot(143);hold on;
gscatter(X_1*1e3,X_4,group_idx,[],[],[],'off');
xlabel('$d_i~$(mm)','fontsize',20,'interpreter','latex');
ylabel('$N_t$','fontsize',20,'interpreter','latex');
set(gca           ,             ...
    'Box'         , 'on'      , ...
    'TickDir'     , 'in'      , ...
    'TickLength'  , [.02 .02] , ...
...
subplot(141);hold on;
f1=gscatter(X_1*1e3,X_2*1e3,group_idx,[],[],[],'off');
xlabel('$d_i~$(mm)','fontsize',20,'interpreter','latex');
...
subplot(142);hold on;
gscatter(X_1*1e3,X_3*1e3,group_idx,[],[],[],'off');
xlabel('$d_i~$(mm)','fontsize',20,'interpreter','latex');
...
subplot(144);hold on;
f1=gscatter(X_1*1e3,X_5,group_idx,[],[],[],'off');
% for k = 1:length(f1)
%     f1(k).MarkerSize = 9;
%     if k == 1
...
% print(['figures/surface_d_i_vs_parameters.fig'],'-deps','-r0','-painters')
savefig(['figures/surface_d_i_vs_parameters.fig'])
