...
f2(N_t,d_i,d_w,L_free) = (Fhat - (2778046668940015*G*d_w^4*(L_free^2 + 4*N_t^2*pi^2*(d_i/2 + d_w/2)^2)^2*(nu + 1)^2*(d_i/2 + d_w/2)^2*(L_free^2 - Lhat^2 + 4*N_t^2*pi^2*(d_i/2 + d_w/2)^2)^2)/(1125899906842624*pi^2*((d_i/2 + d_w/2)^2 + L_free^2/(4*N_t^2*pi^2))^2*((nu + 1)*(L_free^2 - Lhat^2 + 4*N_t^2*pi^2*(d_i/2 + d_w/2)^2)^(3/2) - Lhat*(L_free^2 + 4*N_t^2*pi^2*(d_i/2 + d_w/2)^2)^(1/2)*(L_free - Lhat))^2*(8*L_free^2 - 8*Lhat + 32*N_t^2*pi^2*(d_i/2 + d_w/2)^2)))^2;
...
[xfinal,fval,exitflag,output2] = fmincon(fh3,[7;0.024;0.001;0.1],...
    [],[],[],[],[],[],constraint,options)
double(f1(xfinal(1),xfinal(2),xfinal(3),xfinal(4)))
fval;
