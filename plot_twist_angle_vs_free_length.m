...
% variable
L_free_range = linspace(.001,L_free*10,1000);

for i = 1:length(L_free_range)
% conversion
[Conversion_Output] = ...
  Convert_Build_Params(d_w, L_free_range(i), end_condition, N_t, L_free);
...
% figure
figure();hold on;
plot(L_free_range, theta*180/pi,'r.','linewidth',2);
xlabel(['$L_{free}~(m)$'], 'FontSize', 20 , 'interpreter', 'latex')
...
print(['figures/theta_vs_L_free'],'-depsc','-r0','-painters')
print(['figures/theta_vs_L_free'],'-dpng','-r0','-painters')
