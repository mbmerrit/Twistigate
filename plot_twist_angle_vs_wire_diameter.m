...
% variable
d_w_range = linspace(.001,d_w*100,1000);

for i = 1:length(d_w_range)
% conversion
[Conversion_Output] = ...
  Convert_Build_Params(d_w, d_w_range(i), end_condition, N_t, L_free);
...
% figure
figure();hold on;
plot(d_w_range, theta*180/pi,'r.','linewidth',2);
xlabel('$d_w$~(m)', 'FontSize', 20 , 'interpreter', 'latex')
...
print(['figures/theta_vs_d_w'],'-depsc','-r0','-painters')
print(['figures/theta_vs_d_w'],'-dpng','-r0','-painters')
