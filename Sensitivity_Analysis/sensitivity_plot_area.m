...
% This code contributes to the sensitivity analysis. Choosing a
% variety of different springs, the code plots area curves
% indicating the relative change in twist angle compared to the
% relative change in each design parameter.
...
figure;
% z = area(linspace(-25,25,101),([(min(abs(Lfhat_p)).*sign(Lfhat_p(1,:)))' (range(Lfhat_p).*sign(Lfhat_p(1,:)))'])); hold on;
% z(1).FaceColor = 'none'; z(1).LineStyle = 'none'; z(2).FaceAlpha = .5;
...
ylabel('%\Delta \theta')
legend([a(2) b(2) c(2) d(2) e(2) f(2)],l,'location','northeast','Orientation','horizontal','interpreter','latex');